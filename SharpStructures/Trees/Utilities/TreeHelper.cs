...
namespace SharpStructures.Trees.Utilities
{
        ...
        where TNode : TreeNode<T, TNode>
    {
        ...
        public static bool IsValidRec(BinarySearchTree<T> tree, BSTNode<T>? node)
        {
            ...
            if (node!.Left && tree.Comparator.Compare(tree.Max(node.Left)!.Value, node.Value) >= 0)
                return false;
            if (node.Right && tree.Comparator.Compare(tree.Max(node.Right)!.Value, node.Value) < 0)
                return false;
        ...
        public static bool IsValidRec(RBTNode<T>? node)
        {
            ...
            {
                if (node.Left && node.Left!.Type == NodeType.Red) return false;
                if (node.Right && node.Right!.Type == NodeType.Red) return false;
            }
